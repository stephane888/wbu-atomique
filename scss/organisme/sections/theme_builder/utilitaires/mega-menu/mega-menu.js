 ...
 */
class HbkMegaMenu {
  constructor(hbkMenuHtml) {
    this.timerScroll;
    this.timerResize;
    this.hbkMenuHtml = hbkMenuHtml;
  }
   ...
   */
  addIconSvgOnItem() {
    const items = this.hbkMenuHtml.querySelectorAll('.item');
    items.forEach((item) => {
  ...
  addEventListenerToSvg(svgElement) {
    const container = this.hbkMenuHtml;
   ...
   */
  addIconClose() {
    const MainMenu = this.hbkMenuHtml;
  ...
  listernerScroll() {
    ...
    function faireQuelqueChose(directionDown) {
      ...
      if (actifStatic) {
        ...
        const ht = containerMenu.getAttribute('data-fixed-top')
          ? parseInt(containerMenu.getAttribute('data-fixed-top')) + height
          : height;
        if (position.top <= -ht) {
