...
class ReorderGatewaysCampaign
  def initialize(desired_order, country_sort)
  ...
  def run(cart, payment_gateways)
    ...
    return unless country_selector.match?(address.country_code)
...
CAMPAIGNS = [
  ReorderGatewaysCampaign.new(DESIRED_GATEWAY_ORDER, COUNTRY_SORT),
]
