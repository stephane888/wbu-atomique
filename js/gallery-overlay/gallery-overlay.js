...
class GalleryOverlay {
  ...
  build() {
    ...
    generatedElement.imagesContainer.classList.add(
    ...
    const section = document.querySelector(
      settings.section.selector + settings.section.value
    );
    const imagesOverlay = document.querySelectorAll(
      settings.image_overlay.selector + settings.image_overlay.value
    );
    const gallerySelector =
      settings.galleryContainer.selector + settings.galleryContainer.value;
    for (const key in generatedElement) {
      if (settings[key]) {
        if (settings[key].type === "class" && settings[key].value) {
          ...
          generatedElement[key].setAttribute(
            settings[key].type,
            settings[key].value
          );
        }
    ...
    const elementSelector =
      settings.galleryElement.selector + settings.galleryElement.value;

    generatedElement.nextButton.addEventListener("click", function (event) {
      let imgRender = generatedElement.image.classList.contains("show")
        ? generatedElement.image
        : generatedElement.image2;
      ...
      const nextElement =
        currentImg.closest(elementSelector).nextElementSibling;
      if (nextElement) {
        const nextImg = nextElement.querySelector("a");
        imgRender = switchShowElement(
          generatedElement.image,
          generatedElement.image2
        );
    ...
    generatedElement.prevButton.addEventListener("click", function (event) {
      ...
      const prevElement =
        currentImg.closest(elementSelector).previousElementSibling;
      if (prevElement) {
        ...
        imgRender.setAttribute(
          "src",
          images[images.length - 1].getAttribute("href")
        );
